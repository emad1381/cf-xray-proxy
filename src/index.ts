...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Strict Path Whitelisting Logic
    const isSystemPath =
      isHealthEndpoint(request, requestUrl.pathname, env) ||
      isStatusEndpoint(request, requestUrl.pathname) ||
      isLandingPageRequest(request, requestUrl.pathname) ||
      (subscriptionConfig.enabled && !!subscriptionRoute) ||
      (subscriptionConfig.enabled && isRootPath);
    ...
    // const isUpgrade = isUpgradeRequest(forwardedRequest); // Already calculated above
